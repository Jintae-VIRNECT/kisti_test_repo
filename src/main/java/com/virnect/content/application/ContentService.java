...
public class ContentService {
    ...
    public ApiResponse<ContentUploadResponse> contentDuplicate(
        ...
        //TODO: 컨텐츠:타켓 = N:1 이되면 수정해야 함.
        //컨텐츠 복제할 때 타겟까지 함께 복제 하는 것으로 수정. 타겟과  contents는 1:1 이라 함.(현재까지는)
        Target target = content.getTargetList().get(0);
        ...
        List<ContentTargetResponse> contentTargetResponseList = new ArrayList<>();
        ContentTargetResponse contentTargetResponse = new ContentTargetResponse();
        contentTargetResponse.setId(target.getId());
        contentTargetResponse.setData(target.getData());
        contentTargetResponse.setType(target.getType());
        contentTargetResponse.setImgPath(target.getImgPath());
        contentTargetResponse.setSize(target.getSize());
        contentTargetResponseList.add(contentTargetResponse);
