...
public class CustomPersistentTokenRepository implements PersistentTokenRepository {
    ...
    public void createNewToken(PersistentRememberMeToken token) {
        RememberMeToken rememberToken = new RememberMeToken(token.getUsername(), token.getSeries(), token.getTokenValue(), token.getDate());
    ...
    public void updateToken(String series, String tokenValue, Date date) {
        RememberMeToken rememberMeToken = rememberMeTokenRepository.findById(series).orElseThrow(() ->
                new RememberMeAuthenticationException("Update RememberMe Token - remember me token not found by series ["+series+"]"));
    ...
    public PersistentRememberMeToken getTokenForSeries(String series) {
        RememberMeToken rememberMeToken = rememberMeTokenRepository.findById(series).orElseThrow(() ->
                new RememberMeAuthenticationException("Get Token For Series - remember me token not found by series ["+series+"]"));
