...
const createToast = function(html, options) {
  ...
  if (Array.isArray(options.action)) {
    options.action.forEach(action => {
      ...
      if (el) {
        if (toast.querySelector('.toasted__buttons')) {
          toast.querySelector('.toasted__buttons').appendChild(el)
        } else {
    ...
    if (action) {
      if (toast.querySelector('.toasted__buttons')) {
        toast.querySelector('.toasted__buttons').appendChild(action)
      } else {
...
const createIcon = (options, toast) => {
  ...
  if (options.icon) {
    ...
    switch (options.iconPack) {
      ...
      case 'callback':
        let callback =
          options.icon && options.icon instanceof Function ? options.icon : null
