...
export default {
  install(Vue, { Store }) {
    ...
    const httpPostRequest = (url, body, errorMsg, callback) => {
      ...
      http.open('POST', 'https://media.virnect.com:5001/' + url, true)
    ...
    const httpGetRequest = (url, errorMsg, callback) => {
      console.log(url)
      var http = new XMLHttpRequest()
      http.open('GET', 'https://media.virnect.com:5001/' + url, true)
      http.setRequestHeader('Content-type', 'application/json')
      http.addEventListener('readystatechange', processRequest, false)
      http.send()
    ...
    Vue.prototype.$openvidu = {
              ...
              .then(() => {
                ...
                setTimeout(() => {
                  ...
                  publisher.on('streamCreated', event => {
                    ...
                    console.log(
                      document.querySelector('#video-view__main video'),
                    )
                    document.querySelector(
                      '#video-view__main video',
                    ).muted = true
                    console.log(
                      document.querySelector('#video-view__main video').muted,
                    )
      ...
      disconnect: id => {
        return new Promise((resolve, reject) => {
          ...
          session
            .forceDisconnect({
              connectionId: subscribers[idx].stream.connection.connectionId,
            })
            .then(() => {
              resolve()
            })
