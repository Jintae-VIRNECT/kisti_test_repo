...
var WebRtcStats = /** @class */ (function() {
  ...
  WebRtcStats.prototype.sendStatsToHttpEndpoint = function(instrumentation) {
          ...
          if (
            ...
            if (stat.mediaType === 'video') {
              metrics['framesDecoded'] =
                (stat.framesDecoded - _this.stats.inbound.video.framesDecoded) /
                _this.statsInterval
              metrics['nackCount'] =
            ...
            if (
              ...
              if (stat.mediaType === 'video') {
                metrics['framesDecoded'] =
                  ...
                  _this.statsInterval
                metrics['nackCount'] =
                  (stat.googNacksSent - _this.stats.inbound.video.nackCount) /
                  _this.statsInterval
                units['framesDecoded'] = 'frames'
                units['nackCount'] = 'packets'
                _this.stats.inbound.video.framesDecoded = stat.framesDecoded
    ...
    this.getStatsAgnostic(this.stream.getRTCPeerConnection(), f, function(
      error,
    ) {
      console.log(error)
    })
