...
var Publisher = /** @class */ (function(_super) {
  ...
  function Publisher(targEl, properties, openvidu) {
    var _this =
      _super.call(
        this,
        new Stream_1.Stream(
          openvidu.session ? openvidu.session : new Session_1.Session(openvidu),
  ...
  Publisher.prototype.replaceTrack = function(track) {
    ...
    var replaceMediaStreamTrack = function() {
      var mediaStream = _this.stream.displayMyRemote()
        ? _this.stream.localMediaStreamWhenSubscribedToRemote
            ...
            _this.videoReference.addEventListener('loadedmetadata', function() {
              ...
              _this.screenShareResizeInterval = setInterval(function() {
                var firefoxSettings = mediaStream
                  ...
                  .getSettings()
                var newWidth =
                  platform.name === 'Chrome' || platform.name === 'Opera'
        ...
        if (_this.stream.isSendScreen() && _this.stream.isSendAudio()) {
          ...
          navigator.mediaDevices
            ...
            ['catch'](function(error) {
              ...
              errorCallback(
                _this.openvidu.generateAudioDeviceError(error, constraints),
              )
              return
            })
        } else {
        ...
        switch (error.name.toLowerCase()) {
          case 'notfounderror':
            navigator.mediaDevices
              ...
              ['catch'](function(e) {
                errorName =
                  OpenViduError_1.OpenViduErrorName.INPUT_VIDEO_DEVICE_NOT_FOUND
                errorMessage = error.toString()
                errorCallback(
                  new OpenViduError_1.OpenViduError(errorName, errorMessage),
                )
      ...
      _this.openvidu
        ...
        .then(function(myConstraints) {
          ...
          if (
            ...
            successCallback(
              _this.openvidu.addAlreadyProvidedTracks(
                myConstraints,
                new MediaStream(),
              ),
            )
