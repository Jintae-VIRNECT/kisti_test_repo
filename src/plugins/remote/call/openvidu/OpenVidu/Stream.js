...
var Stream = /** @class */ (function(_super) {
  ...
  function Stream(session, options) {
    ...
    if (options.hasOwnProperty('id')) {
      ...
      if (_this.hasVideo) {
        ...
        _this.typeOfVideo = !_this.inboundStreamOpts.typeOfVideo
          ? undefined
          : _this.inboundStreamOpts.typeOfVideo
        _this.frameRate =
          _this.inboundStreamOpts.frameRate === -1
            ? undefined
            : _this.inboundStreamOpts.frameRate
        _this.videoDimensions = _this.inboundStreamOpts.videoDimensions
      }
      if (
        !!_this.inboundStreamOpts.filter &&
        Object.keys(_this.inboundStreamOpts.filter).length > 0
      ) {
        if (
          !!_this.inboundStreamOpts.filter.lastExecMethod &&
          Object.keys(_this.inboundStreamOpts.filter.lastExecMethod).length ===
            0
        ) {
      ...
      if (_this.hasAudio) {
        _this.audioActive = !!_this.outboundStreamOpts.publisherProperties
          .publishAudio
      }
      if (_this.hasVideo) {
        _this.videoActive = !!_this.outboundStreamOpts.publisherProperties
          .publishVideo
        _this.frameRate = _this.outboundStreamOpts.publisherProperties.frameRate
        if (
          typeof MediaStreamTrack !== 'undefined' &&
          _this.outboundStreamOpts.publisherProperties.videoSource instanceof
            MediaStreamTrack
        ) {
  ...
  Stream.prototype.on = function(type, handler) {
    _super.prototype.onAux.call(
      this,
      type,
      "Event '" + type + "' triggered by stream '" + this.streamId + "'",
      handler,
    )
    return this
  }
  ...
  Stream.prototype.once = function(type, handler) {
    _super.prototype.onceAux.call(
      this,
      type,
      "Event '" + type + "' triggered once by stream '" + this.streamId + "'",
      handler,
    )
    return this
  }
      ...
      _this.session.openvidu.sendRequest(
        ...
        function(error, response) {
          if (error) {
            ...
            logger.info(
              'Filter successfully removed from Stream ' + _this.streamId,
            )
            var oldValue = _this.filter
            delete _this.filter
            _this.session.emitEvent('streamPropertyChanged', [
  ...
  Stream.prototype.isSendScreen = function() {
    ...
    if (platform.name === 'Electron') {
      screen =
        typeof this.outboundStreamOpts.publisherProperties.videoSource ===
          'string' &&
  ...
  Stream.prototype.streamIceConnectionStateBroken = function() {
    ...
    if (
      ...
      logger.warn(
        'OpenVidu Browser advanced configuration option "forceMediaReconnectionAfterNetworkDrop" is enabled. Publisher stream ' +
          this.streamId +
    ...
    if (this.mediaStream) {
      if (!this.speechEvent) {
        ...
        harkOptions.interval =
          typeof harkOptions.interval === 'number' ? harkOptions.interval : 100
  ...
  Stream.prototype.initWebRtcPeerSend = function(reconnect) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var successCallback = function(sdpOfferParam) {
        ...
        _this.session.openvidu.sendRequest(method, params, function(
          error,
          response,
        ) {
          if (error) {
            ...
            _this.webRtcPeer
              ...
              .then(function() {
                ...
                if (_this.displayMyRemote()) {
                  _this.localMediaStreamWhenSubscribedToRemote =
                    _this.mediaStream
      ...
      _this.webRtcPeer
        ...
        ['catch'](function(error) {
          reject(
            new Error('(publish) SDP offer error: ' + JSON.stringify(error)),
          )
        })
    })
  }
    ...
    return new Promise(function(resolve, reject) {
      ...
      var successCallback = function(sdpOfferParam) {
        ...
        _this.session.openvidu.sendRequest(method, params, function(
          ...
          if (error) {
            reject(
              new Error('Error on recvVideoFrom: ' + JSON.stringify(error)),
            )
          } else {
            ...
            _this.webRtcPeer
              .processAnswer(response.sdpAnswer, needsTimeoutOnProcessAnswer)
              .then(function() {
                logger.info(
                  "'Subscriber' (" +
                    _this.streamId +
                    ') successfully ' +
                    (reconnect ? 'reconnected' : 'subscribed'),
                )
                _this.remotePeerSuccessfullyEstablished()
                _this.initWebRtcStats()
                resolve()
      ...
      _this.webRtcPeer
        ...
        ['catch'](function(error) {
          reject(
            new Error('(subscribe) SDP offer error: ' + JSON.stringify(error)),
          )
        })
    })
  }
