;(function() {
  ...
  var isSearch = function(searchStr, targetStr, jungsungExactlyMatch) {
    ...
    var searchStrArr =
      typeof searchStr === 'string' ? searchStr.split('') : searchStr
    ...
    var targetStrChosung = targetStr
      .split('')
        ...
        find = parsedArr.every(function(p, pIndex) {
          if (p.isComplete) {
            if (p.endsWithConsonant) {
              ...
              if (
                jungsungExactlyMatch === true &&
                p.jungsungArr.length + 1 !== disassembleTargetStr.length
              ) {
                return false
              }
