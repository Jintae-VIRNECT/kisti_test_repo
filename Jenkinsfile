      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
              sh 'if [ `docker images | grep pf-webaccount | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f $(docker images | grep "pf-webaccount" | grep -v server | grep "latest" | awk \'{print $3}\'); else echo "Just One Images..."; fi;'
            }
          }
        }
