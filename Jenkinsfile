            ...
            parallel {
                stage('Develop Branch') {
                    ...
                    steps {
                        ...
                        catchError {
                             sh 'if [ `docker images | grep pf-webdownload | grep -v "103505534696" | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f $(docker images | grep "pf-webdownload" | grep -v server | grep "latest" | awk \'{print $3}\'); else echo "Just One Images..."; fi;'
                        }
                    }
                }
                    ...
                    steps {
                        script {
                            docker.withRegistry("https://$aws_ecr_address", 'ecr:ap-northeast-2:aws-ecr-credentials') {
                                ...
                                docker.image("pf-webdownload:${GIT_TAG}").push("latest")
                            }
                        }
                                ...
                                publishers: [
                                    sshPublisherDesc(
                                        ...
                                        transfers: [
                                            ...
                                            sshTransfer(
                                                execCommand: "docker pull $aws_ecr_address/pf-webdownload:\\${GIT_TAG}"
                                            ),
                                            sshTransfer(
                                            ...
                                            sshTransfer(
                                                execCommand: "if [ `docker images | grep pf-webdownload | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f \$(docker images | grep \"pf-webdownload\" | grep -v server | grep -v \\${GIT_TAG} | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                                            )
                                        ]
                                    )
                                ]
