    ...
    stage('Deploy') {
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            sh 'wget http://localhost:8084/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_old.json'
            ...
            sh 'wget http://localhost:8084/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_new.json'
            ...
            catchError {
               sh "if [ `docker images | grep pf-message | grep -v 103505534696 | wc -l` -gt 3 ]; then docker rmi  -f \$(docker images | grep \"pf-message\" | grep -v \\${GIT_TAG} | grep -v \"latest\"  | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
                ...
                publishers: [
                  sshPublisherDesc(
                    ...
                    transfers: [
                      ...
                      sshTransfer(
                        execCommand: "if [ `docker images | grep pf-message | wc -l` -ne 1 ]; then docker rmi  -f \$(docker images | grep \"pf-message\" | grep -v \\${GIT_TAG} | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                      )
                    ]
                  )
                ]
                ...
                publishers: [
                  sshPublisherDesc(
                    ...
                    transfers: [
                      ...
                      sshTransfer(
                        execCommand: "docker run -p 8084:8084 --restart=always -e 'CONFIG_SERVER=http://3.35.50.181:6383' -e 'VIRNECT_ENV=onpremise' -e eureka.instance.ip-address=`hostname -I | awk  \'{print \$1}\'` -d --name=pf-message $aws_ecr_address/pf-message:\\${GIT_TAG}"
                      ),
                      sshTransfer(
                        execCommand: "if [ `docker images | grep pf-message | wc -l` -ne 1 ]; then docker rmi  -f \$(docker images | grep \"pf-message\" | grep -v \\${GIT_TAG} | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                      )
