    ...
    stage('Deploy') {
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            sh 'wget http://localhost:8079/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_old.json'
            ...
            sh 'wget http://localhost:8079/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_new.json'
            ...
            catchError() {
               sh "if [ `docker images | grep pf-processmanagement | grep -v 103505534696 | wc -l` -gt 3 ]; then docker rmi  -f \$(docker images | grep \"pf-processmanagement\" | grep -v \\${GIT_TAG} | grep -v \"latest\" | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
