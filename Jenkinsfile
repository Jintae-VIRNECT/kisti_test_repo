      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
              sh "if [ `docker images | grep pf-webaccount | grep -v 103505534696 | grep -v server | wc -l` -gt 2 ]; then docker rmi  -f \$(docker images | grep \"pf-webaccount\" | grep -v server | grep -v \\${GIT_TAG} | grep -v \"latest\" | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
                ...
                publishers: [
                  sshPublisherDesc(
                    ...
                    transfers: [
                      ...
                      sshTransfer(
                        execCommand: "docker run -p 8822:8822 --restart=always -e 'CONFIG_SERVER=http://3.35.50.181:6383' -e 'VIRNECT_ENV=onpremise' -d --name=pf-webaccount $aws_ecr_address/pf-webaccount:\\${GIT_TAG}"
                      ),
                      sshTransfer(
                        execCommand: "if [ `docker images | grep pf-webaccount | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f \$(docker images | grep \"pf-webaccount\" | grep -v server | grep -v \\${GIT_TAG} | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                      )
