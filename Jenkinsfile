      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
               sh 'if [ `docker images | grep pf-contentsmanagement | grep -v 103505534696 | wc -l` -ne 1 ]; then docker rmi  -f $(docker images | grep "pf-contentsmanagement" | grep "latest" | awk \'{print $3}\'); else echo "Just One Images..."; fi;'
            }
          }
        }
                ...
                publishers: [
                  sshPublisherDesc(
                    ...
                    transfers: [
                      ...
                      sshTransfer(
                        execCommand: 'if [ `docker images | grep pf-contentsmanagement | wc -l` -ne 1 ]; then docker rmi  -f $(docker images | grep "pf-contentsmanagement" | grep -v ${GIT_TAG} | awk \'{print $3}\'); else echo "Just One Images..."; fi;'
                      )
                    ]
                  )
                ]
