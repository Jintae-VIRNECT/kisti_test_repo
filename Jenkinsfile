      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
              sh 'if [ `docker images | grep rm-web | grep -v 103505534696 | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f $(docker images | grep "rm-web" | grep -v server | grep -v "latest" | awk \'{print $3}\'); else echo "Just One Images..."; fi;'
            }
          }  
        }
