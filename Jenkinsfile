      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
               sh "if [ `docker images | grep pf-message | grep -v 103505534696 | wc -l` -ge 3 ]; then docker rmi  -f \$(docker images | grep \"pf-message\" | grep -v \\${GIT_TAG} | grep -v \"latest\"  | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
