        ...
        stage('Deploy') {
            parallel {
                stage('Develop Branch') {
                    ...
                    steps {
                        sh 'wget -t 20 http://localhost:8000/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_old.json'
                        ...
                        sh 'wget -t 20 http://localhost:8000/v2/api-docs -O /var/lib/jenkins/Swagger-Diff/Diff/${SERVICE_NAME}_new.json'
                        ...
                        catchError {
                             sh "if [ `docker images | grep rm-service | grep -v 103505534696 | grep -v server | wc -l` -gt 5 ]; then docker rmi  -f \$(docker images | grep \"rm-service\" | grep -v server | grep -v \\${GIT_TAG} | grep -v \"latest\" | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                        }
                    }
                }
                                ...
                                publishers: [
                                    sshPublisherDesc(
                                        ...
                                        transfers: [
                                            ...
                                            sshTransfer(
                                                execCommand: "docker run -p 8000:8000 --restart=always -e 'CONFIG_SERVER=http://3.35.50.181:6383' -e 'VIRNECT_ENV=onpremise' -e eureka.instance.ip-address=`hostname -I | awk  \'{print \$1}\'` -d --name=rm-service $aws_ecr_address/rm-service:\\${GIT_TAG}"
                                            ),
                                            sshTransfer(
                                                execCommand: "if [ `docker images | grep rm-service | grep -v server | wc -l` -ne 1 ]; then docker rmi  -f \$(docker images | grep \"rm-service\" | grep -v server | grep -v \\${GIT_TAG} | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
                                            )
