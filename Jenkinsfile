      ...
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError {
               sh "if [ `docker images | grep pf-webworkstation | grep -v 103505534696 | grep -v server | grep -v 192.168.6.4 | wc -l` -gt 3 ]; then docker rmi   \$(docker images | grep \"pf-webworkstation\" | grep -v server | grep -v \\${GIT_TAG} | grep -v \"latest\" | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
