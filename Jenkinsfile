    ...
    stage('Build') {
      parallel {
        stage('Develop Branch') {
          ...
          steps {
            sh 'docker build -t pf-download -f docker/Dockerfile .'
          }
        }
        ...
        stage('Staging Branch') {
          ...
          steps {
            catchError() {
              ...
              sh 'docker build -t pf-download:${GIT_TAG} -f docker/Dockerfile .'
            }
          }
        }
      }
      ...
      parallel {
        ...
        stage('Develop Branch') {
          ...
          steps {
            ...
            catchError() {
               sh "if [ `docker images | grep pf-download | grep -v 103505534696 | grep -v server | wc -l` -gt 2 ]; then docker rmi  -f \$(docker images | grep \"pf-download\" | grep -v \\${GIT_TAG} | grep -v \"latest\" | awk \'{print \$3}\'); else echo \"Just One Images...\"; fi;"
            }
          }
        }
          ...
          steps {
            script {
              docker.withRegistry("https://$aws_ecr_address", 'ecr:ap-northeast-2:aws-ecr-credentials') {
                docker.image("pf-download:${GIT_TAG}").push("${GIT_TAG}")
                docker.image("pf-download:${GIT_TAG}").push("latest")
              }
            }
