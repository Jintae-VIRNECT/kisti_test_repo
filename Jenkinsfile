pipeline {
    agent any
      environment {
        GIT_TAG = sh(returnStdout: true, script: 'git for-each-ref refs/tags --sort=-createdate --format="%(refname)" --count=1 | cut -d/  -f3').trim()
            ...
            parallel {
                ...
                stage('Develop Branch') {
                    ...
                    steps {
                        ...
                        sh 'docker run -p 8000:8000 --restart=always -e "SPRING_PROFILES_ACTIVE=develop" -d --name=rm-service rm-service'
