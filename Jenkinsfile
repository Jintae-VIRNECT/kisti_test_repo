            ...
            catchError() {
              ...
              script {
                sshPublisher(
                  ...
                  sh 'curl -s -d  ""{"tag_name": "$GIT_TAG", "target_commitish": "master", "name": "$NAME","body": "$DESCRIPTION","draft"\: false,"prerelease": false}" "https://api.github.com/repos/$REPO_NAME/releases?access_token=$securitykey"'
