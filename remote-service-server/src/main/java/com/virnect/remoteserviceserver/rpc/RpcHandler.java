...
package com.virnect.remoteserviceserver.rpc;
...
import com.virnect.remoteserviceserver.utils.GeoLocationByIp;
...
public class RpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	public void handleRequest(Transaction transaction, Request<JsonObject> request) throws Exception {
		...
		if (ProtocolElements.JOINROOM_METHOD.equals(request.getMethod())) {
			...
			throw new RemoteServiceException(Code.TRANSPORT_ERROR_CODE,
	...
	private void joinRoom(RpcConnection rpcConnection, Request<JsonObject> request) {
		...
		final com.virnect.remoteserviceserver.core.Session session = sessionManager.getSessionWithNotActive(sessionId);
		if (session == null) {
			log.error("ERROR: Session {} not found", sessionId);
			throw new RemoteServiceException(Code.ROOM_NOT_FOUND_ERROR_CODE,
		...
		if (remoteServiceConfig.isOpenViduSecret(secret)) {
			...
			try {
				...
				throw new RemoteServiceException(Code.TOKEN_CANNOT_BE_CREATED_ERROR_CODE,
		...
		if (tokenObj != null) {
			...
			if (sessionManager.formatChecker.isServerMetadataFormatCorrect(clientMetadata)) {
				...
				if (session.closingLock.readLock().tryLock()) {
					try {
						if (session.isClosed()) {
							throw new RemoteServiceException(Code.ROOM_CLOSED_ERROR_CODE,
				...
				throw new RemoteServiceException(Code.USER_METADATA_FORMAT_INVALID_ERROR_CODE, "Unable to join session "
						+ sessionId + ". The metadata received from the client-side has an invalid format");
			}
		} else {
			log.error("ERROR: token not valid");
			throw new RemoteServiceException(Code.USER_UNAUTHORIZED_ERROR_CODE,
	...
	public void afterConnectionClosed(Session rpcSession, String status) throws Exception {
		...
		if (!message.isEmpty()) {
			...
			if (rpc != null && rpc.getSessionId() != null) {
				com.virnect.remoteserviceserver.core.Session session = this.sessionManager.getSession(rpc.getSessionId());
	...
	private Participant sanityCheckOfSession(RpcConnection rpcConnection, String methodName) throws RemoteServiceException {
		...
		if (sessionId == null) { // null when afterConnectionClosed
			...
			throw new RemoteServiceException(Code.GENERIC_ERROR_CODE, errorMsg);
		} else {
