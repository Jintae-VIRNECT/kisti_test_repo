...
package com.virnect.remoteserviceserver.config;

import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonSyntaxException;
import com.virnect.java.client.RemoteService;
...
import com.virnect.remoteserviceserver.recording.RecordingNotification;
...
public class RemoteServiceConfig {
	...
	private void calculatePublicUrl() {
		...
		if (publicUrl.startsWith("https://")) {
			RemoteServiceServerApplication.wsUrl = publicUrl.replace("https://", "wss://");
		} else if (publicUrl.startsWith("http://")) {
			RemoteServiceServerApplication.wsUrl = publicUrl.replace("http://", "wss://");
		}
		if (RemoteServiceServerApplication.wsUrl.endsWith("/")) {
			RemoteServiceServerApplication.wsUrl = RemoteServiceServerApplication.wsUrl.substring(0, RemoteServiceServerApplication.wsUrl.length() - 1);
		}
		String finalUrl = RemoteServiceServerApplication.wsUrl.replaceFirst("wss://", "https://").replaceFirst("ws://", "http://");
		this.setFinalUrl(finalUrl);
		RemoteServiceServerApplication.httpUrl = this.getFinalUrl();
	}
	...
	protected void populatePropertySourceFromDotenv() {
		...
		if (dotenvFile != null) {
			if (dotenvFile.canRead()) {
				...
				} catch (IOException | Dotenv.DotenvFormatException e) {
