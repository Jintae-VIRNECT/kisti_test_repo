...
import com.virnect.remoteserviceserver.utils.CommandExecutor;
...
public abstract class RecordingService {
	...
	protected void generateRecordingMetadataFile(Recording recording) {
		String folder = this.remoteServiceConfig.getOpenViduRecordingPath() + recording.getId();
		...
		String filePath = this.remoteServiceConfig.getOpenViduRecordingPath() + recording.getId() + "/"
	...
	protected Recording sealRecordingMetadataFileAsStopped(Recording recording) {
		final String entityFile = this.remoteServiceConfig.getOpenViduRecordingPath() + recording.getId() + "/"
				+ RecordingManager.RECORDING_ENTITY_FILE + recording.getId();
		return this.sealRecordingMetadataFile(recording, 0, 0, Status.stopped,
				entityFile);
	}
	...
	protected PropertiesRecordingId setFinalRecordingNameAndGetFreeRecordingId(Session session,
		...
		if (properties.name() == null || properties.name().isEmpty()) {
			...
			if (com.virnect.java.client.Recording.OutputMode.COMPOSED.equals(properties.outputMode())
					&& properties.hasVideo()) {
