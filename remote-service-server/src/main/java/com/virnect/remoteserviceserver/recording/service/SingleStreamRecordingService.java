...
import com.virnect.remoteserviceserver.recording.RecorderEndpointWrapper;
...
public class SingleStreamRecordingService extends RecordingService {
			...
			if (participant.singleRecordingLock.tryLock(15, TimeUnit.SECONDS)) {
				try {
					...
					RecorderEndpoint recorder = new RecorderEndpoint.Builder(pipeline,
							"file://" + this.remoteServiceConfig.getOpenViduRecordingPath() + recordingId + "/"
	...
	private MediaProfileSpecType generateMediaProfile(RecordingProperties properties, Participant participant)
		...
		if (streamHasAudio) {
			...
			if (propertiesHasVideo) {
				...
				throw new RemoteServiceException(Code.MEDIA_TYPE_STREAM_INCOMPATIBLE_WITH_RECORDING_PROPERTIES_ERROR_CODE,
	...
	private void commonWriteIndividualMetadataFile(RecorderEndpointWrapper wrapper,
		...
		String filesPath = this.remoteServiceConfig.getOpenViduRecordingPath() + wrapper.getRecordingId() + "/";
	...
	private Recording sealMetadataFiles(Recording recording) {
		...
		String folderPath = this.remoteServiceConfig.getOpenViduRecordingPath() + recording.getId() + "/";
