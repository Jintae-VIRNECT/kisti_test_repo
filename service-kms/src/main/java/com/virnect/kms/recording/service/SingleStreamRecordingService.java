...
import com.virnect.kms.core.EndReason;
import com.virnect.kms.core.Participant;
import com.virnect.kms.core.Session;
...
import com.virnect.kms.recording.RecorderEndpointWrapper;
import com.virnect.kms.recording.Recording;
import com.virnect.kms.recording.RecordingDownloader;
import com.virnect.kms.utils.QuarantineKiller;
...
public class SingleStreamRecordingService extends RecordingService {
			...
			if (participant.singleRecordingLock.tryLock(15, TimeUnit.SECONDS)) {
				try {
					...
					RecorderEndpoint recorder = new RecorderEndpoint.Builder(pipeline,
							"file://" + remoteServiceConfig.remoteServiceProperties.getRemoteServiceRemoteRecordingPath() + recordingId + "/"
	...
	private void commonWriteIndividualMetadataFile(RecorderEndpointWrapper wrapper,
		...
		String filesPath = this.remoteServiceConfig.remoteServiceProperties.getRemoteServiceRecordingPath() + wrapper.getRecordingId() + "/";
	...
	private Recording sealMetadataFiles(Recording recording) {
		...
		String folderPath = this.remoteServiceConfig.remoteServiceProperties.getRemoteServiceRecordingPath() + recording.getId() + "/";
