...
public class KurentoSession extends Session {
	...
	public void join(Participant participant) {
		...
		KurentoParticipant kurentoParticipant = new KurentoParticipant(participant, this, this.kurentoEndpointConfig, this.remoteServiceConfig, this.recordingManager);
	...
	@Override
	public void leave(String participantPrivateId, EndReason reason) throws RemoteServiceException {

		checkClosed();

		KurentoParticipant participant = (KurentoParticipant) participants.get(participantPrivateId);
		if (participant == null) {
			throw new RemoteServiceException(Code.USER_NOT_FOUND_ERROR_CODE, "Participant with private id "
	...
	private void createPipeline() {
		synchronized (pipelineCreateLock) {
			...
			if (getPipeline() == null) {
				...
				throw new RemoteServiceException(Code.ROOM_CANNOT_BE_CREATED_ERROR_CODE, message);
			}
		...
		this.closePipeline(() -> {
			...
			try {
				...
				getParticipants().forEach(p -> {
					if (!RemoteServiceRole.SUBSCRIBER.equals(p.getToken().getRole())) {
