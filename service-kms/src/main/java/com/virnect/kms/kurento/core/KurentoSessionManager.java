...
package com.virnect.kms.kurento.core;
...
import com.virnect.kms.core.*;
import com.virnect.kms.kurento.endpoint.KurentoFilter;
import com.virnect.kms.kurento.endpoint.PublisherEndpoint;
import com.virnect.kms.kurento.endpoint.SdpType;
import com.virnect.kms.kurento.kms.Kms;
import com.virnect.kms.kurento.kms.KmsManager;
import com.virnect.kms.rpc.RpcHandler;
import com.virnect.kms.session.ServiceSessionManager;
import com.virnect.kms.utils.GeoLocation;
import com.virnect.kms.utils.JsonUtils;
					...
					if (KmsManager.selectAndRemoveKmsLock.tryLock(KmsManager.MAX_SECONDS_LOCK_WAIT, TimeUnit.SECONDS)) {
						try {
							...
							if (kSession == null) {
								...
								//flags-create
								serviceSessionManager.createSession(sessionNotActive);
			...
			try {
				if (kSession.joinLeaveLock.tryLock(15, TimeUnit.SECONDS)) {
					try {
						...
						//flags-join
						serviceSessionManager.joinSession(participant, sessionId, existingParticipants, transactionId);
		...
		try {
			if (session.joinLeaveLock.tryLock(15, TimeUnit.SECONDS)) {
				try {
					...
					//flags-leave
					serviceSessionManager.leaveSession(participant, sessionId, remainingParticipants, transactionId, reason);
								...
								try {
									if (session.closingLock.writeLock().tryLock(15, TimeUnit.SECONDS)) {
										try {
											...
											this.serviceSessionManager.destroySession(session, reason);
										} finally {
