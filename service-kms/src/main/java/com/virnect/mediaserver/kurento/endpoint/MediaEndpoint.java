...
package com.virnect.mediaserver.kurento.endpoint;
...
import com.virnect.mediaserver.kurento.core.KurentoParticipant;
...
public abstract class MediaEndpoint {
	...
	public MediaEndpoint(EndpointType endpointType, KurentoParticipant owner, String endpointName,
			MediaPipeline pipeline, MediaServerProperties mediaServerProperties, Logger log) {
		if (log == null) {
		...
		if (kurentoTokenOptions != null) {
			this.maxRecvKbps = kurentoTokenOptions.getVideoMaxRecvBandwidth() != null
					...
					: this.mediaServerProperties.bandwidthProperty.getStreamsVideoMaxRecvBandwidth();
			this.minRecvKbps = kurentoTokenOptions.getVideoMinRecvBandwidth() != null
					? kurentoTokenOptions.getVideoMinRecvBandwidth()
					: this.mediaServerProperties.bandwidthProperty.getStreamsVideoMinRecvBandwidth();
			this.maxSendKbps = kurentoTokenOptions.getVideoMaxSendBandwidth() != null
					? kurentoTokenOptions.getVideoMaxSendBandwidth()
					: this.mediaServerProperties.bandwidthProperty.getStreamsVideoMaxSendBandwidth();
			this.minSendKbps = kurentoTokenOptions.getVideoMinSendBandwidth() != null
					? kurentoTokenOptions.getVideoMinSendBandwidth()
					: this.mediaServerProperties.bandwidthProperty.getStreamsVideoMinSendBandwidth();
		} else {
			this.maxRecvKbps = this.mediaServerProperties.bandwidthProperty.getStreamsVideoMaxRecvBandwidth();
			this.minRecvKbps = this.mediaServerProperties.bandwidthProperty.getStreamsVideoMinRecvBandwidth();
			this.maxSendKbps = this.mediaServerProperties.bandwidthProperty.getStreamsVideoMaxSendBandwidth();
			this.minSendKbps = this.mediaServerProperties.bandwidthProperty.getStreamsVideoMinSendBandwidth();
		}
	}
		...
		if (this.isWeb()) {
			...
			builder.buildAsync(new Continuation<WebRtcEndpoint>() {
				...
				public void onSuccess(WebRtcEndpoint result) throws Exception {
					...
					if (mediaServerProperties.coturnProperty.getCoturnIp() != null && !mediaServerProperties.coturnProperty.getCoturnIp().isEmpty()
							&& mediaServerProperties.coturnProperty.isTurnadminAvailable()) {
						webEndpoint.setStunServerAddress(mediaServerProperties.coturnProperty.getCoturnIp());
	 ...
	 */
	public synchronized void addIceCandidate(IceCandidate candidate) throws RemoteServiceException {
		if (!this.isWeb()) {
			throw new RemoteServiceException(Code.MEDIA_NOT_A_WEB_ENDPOINT_ERROR_CODE, "Operation not supported");
		}
		if (webEndpoint == null) {
	 ...
	 */
	protected String processOffer(String offer) throws RemoteServiceException {
		if (this.isWeb()) {
			if (webEndpoint == null) {
				throw new RemoteServiceException(Code.MEDIA_WEBRTC_ENDPOINT_ERROR_CODE,
			...
			if (endpoint == null) {
				throw new RemoteServiceException(Code.MEDIA_RTP_ENDPOINT_ERROR_CODE,
	 ...
	 */
	protected String processAnswer(String answer) throws RemoteServiceException {
		if (this.isWeb()) {
			if (webEndpoint == null) {
				throw new RemoteServiceException(Code.MEDIA_WEBRTC_ENDPOINT_ERROR_CODE,
	 ...
	 *
	 * @see WebRtcEndpoint#addOnIceCandidateListener(EventListener)
	 * @see KurentoParticipant#sendIceCandidate(String, String, IceCandidate)
	 * @throws RemoteServiceException if thrown, unable to register the listener
	 */
	protected void registerOnIceCandidateEventListener(String senderPublicId) throws RemoteServiceException {
		if (!this.isWeb()) {
			return;
		}
		if (webEndpoint == null) {
	 ...
	 * {@link IceCandidate}s.
	 */
	protected void gatherCandidates() throws RemoteServiceException {
		if (!this.isWeb()) {
			return;
		}
		if (webEndpoint == null) {
	...
	private void internalAddIceCandidate(IceCandidate candidate) throws RemoteServiceException {
		if (webEndpoint == null) {
			throw new RemoteServiceException(Code.MEDIA_WEBRTC_ENDPOINT_ERROR_CODE,
