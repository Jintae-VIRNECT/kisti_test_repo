		...
		try {
				...
				try {
					if (KmsManager.selectAndRemoveKmsLock.tryLock(KmsManager.MAX_SECONDS_LOCK_WAIT, TimeUnit.SECONDS)) {
						try {
							...
							if (kSession == null) {
								...
								try {
									...
									throw new RemoteServiceException(Code.ROOM_CANNOT_BE_CREATED_ERROR_CODE,
						...
						sessionEventsHandler.onParticipantJoined(participant, sessionId, null, transactionId,
								new RemoteServiceException(Code.ROOM_CANNOT_BE_CREATED_ERROR_CODE, error));
						return;
					}
					...
					log.error(error);
					throw new RemoteServiceException(Code.ROOM_CLOSED_ERROR_CODE, error);
				}
			}
			...
			if (kSession.isClosed()) {
				log.warn("'{}' is trying to join session '{}' but it is closing", participant.getParticipantPublicId(),
						sessionId);
				throw new RemoteServiceException(Code.ROOM_CLOSED_ERROR_CODE, "'" + participant.getParticipantPublicId()
			...
			try {
				if (kSession.joinLeaveLock.tryLock(15, TimeUnit.SECONDS)) {
					...
					log.error(
							...
							participant.getParticipantPublicId(), sessionId);
					sessionEventsHandler.onParticipantJoined(participant, sessionId, null, transactionId,
							new RemoteServiceException(Code.GENERIC_ERROR_CODE, "Timeout waiting for Session lock"));
				}
			} catch (InterruptedException e) {
				log.error(
						...
						participant.getParticipantPublicId(), sessionId);
				sessionEventsHandler.onParticipantJoined(participant, sessionId, null, transactionId,
						new RemoteServiceException(Code.GENERIC_ERROR_CODE,
	...
	public void publishVideo(Participant participant, MediaOptions mediaOptions, Integer transactionId)
		...
		if (kurentoOptions.getFilter() != null && kurentoTokenOptions != null) {
			if (kurentoTokenOptions.isFilterAllowed(kurentoOptions.getFilter().getType())) {
				...
				RemoteServiceException e = new RemoteServiceException(Code.FILTER_NOT_APPLIED_ERROR_CODE,
		...
		if (sdpAnswer == null) {
			RemoteServiceException e = new RemoteServiceException(Code.MEDIA_SDP_ERROR_CODE,
	...
	public void unpublishVideo(Participant participant, Participant moderator, Integer transactionId,
		...
		try {
			...
			if (!participant.isStreaming()) {
				...
				throw new RemoteServiceException(Code.USER_NOT_STREAMING_ERROR_CODE,
	...
	public void subscribe(Participant participant, String senderName, String sdpOffer, Integer transactionId) {
		...
		try {
			...
			if (senderParticipant == null) {
				...
				throw new RemoteServiceException(Code.USER_NOT_FOUND_ERROR_CODE,
			...
			if (sdpAnswer == null) {
				throw new RemoteServiceException(Code.MEDIA_SDP_ERROR_CODE,
		...
		if (evictedParticipant != null) {
			...
			if (moderator != null && transactionId != null) {
				this.sessionEventsHandler.onForceDisconnect(moderator, evictedParticipant,
						...
						new RemoteServiceException(Code.USER_NOT_FOUND_ERROR_CODE,
		...
		try {
			...
			if (kurentoFilterJson != null) {
				try {
					...
					throw new RemoteServiceException(Code.FILTER_NOT_APPLIED_ERROR_CODE,
		...
		if (participantPrivateId != null) {
			...
			if (participant != null) {
				if (participant.isIpcam()) {
					throw new RemoteServiceException(Code.USER_GENERIC_ERROR_CODE, "Stream '" + streamId
		...
		if (participantPrivateId != null) {
			...
			if (!publisher.isStreaming()) {
				...
				log.warn(
						...
						moderator != null ? moderator.getParticipantPublicId() : publisher.getParticipantPublicId(),
						publisher.getParticipantPublicId(), session.getSessionId());
				throw new RemoteServiceException(Code.EXISTING_FILTER_ALREADY_APPLIED_ERROR_CODE,
		...
		if (!pub.isListenerAddedToFilterEvent(eventType)) {
			...
			try {
				...
				log.error("Request to addFilterEventListener to stream {} gone wrong. Error: {}", streamId,
						e.getMessage());
				throw new RemoteServiceException(Code.FILTER_EVENT_LISTENER_NOT_FOUND,
