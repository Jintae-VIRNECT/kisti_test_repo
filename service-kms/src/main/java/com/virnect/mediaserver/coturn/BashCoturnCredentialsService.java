...
package com.virnect.mediaserver.coturn;
	...
	private void initialize() {
		try {
			...
			if (response.contains("turnadmin: not found")) {
				...
				log.info("COTURN IP: " + this.mediaServerProperties.coturnProperty.getCoturnIp());
				...
				response = CommandExecutor.execCommand(10000, "/bin/sh", "-c",
						"redis-cli -a " + this.mediaServerProperties.coturnProperty.getCoturnRedisPassword() + " -n "
								+ this.mediaServerProperties.coturnProperty.getCoturnRedisDbname() + " flushdb");
				String response2 = CommandExecutor.execCommand(10000, "/bin/sh", "-c",
						"redis-cli -a " + this.mediaServerProperties.coturnProperty.getCoturnRedisPassword() + " -n "
								+ this.mediaServerProperties.coturnProperty.getCoturnRedisDbname() + " --scan --pattern '*'");
				if ("OK".equals(response) && response2.isEmpty()) {
					log.info("COTURN DB is now empty");
				} else {
					log.error("COTURN DB is not empty");
				}
				this.mediaServerProperties.coturnProperty.setTurnadminAvailable(true);
