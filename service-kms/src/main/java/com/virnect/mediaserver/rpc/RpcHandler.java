...
package com.virnect.mediaserver.rpc;
...
public class RpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	private void joinRoom(RpcConnection rpcConnection, Request<JsonObject> request) {
		...
		final com.virnect.mediaserver.core.Session session = sessionManager.getSessionWithNotActive(sessionId);
		if (session == null) {
		...
		if (mediaServerProperties.serverProperty.isServiceSecret(secret)) {
	...
	public void afterConnectionClosed(Session rpcSession, String status) throws Exception {
		...
		if (!message.isEmpty()) {
			...
			if (rpc != null && rpc.getSessionId() != null) {
				com.virnect.mediaserver.core.Session session = this.sessionManager.getSession(rpc.getSessionId());
