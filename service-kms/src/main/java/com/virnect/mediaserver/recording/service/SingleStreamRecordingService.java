...
import com.virnect.mediaserver.recording.RecorderEndpointWrapper;
...
public class SingleStreamRecordingService extends RecordingService {
			...
			if (participant.singleRecordingLock.tryLock(15, TimeUnit.SECONDS)) {
				try {
					...
					RecorderEndpoint recorder = new RecorderEndpoint.Builder(pipeline,
							"file://" + this.mediaServerProperties.recordingProperty.getRecordingPath() + recordingId + "/"
	...
	private void commonWriteIndividualMetadataFile(RecorderEndpointWrapper wrapper,
		...
		String filesPath = this.mediaServerProperties.recordingProperty.getRecordingPath() + wrapper.getRecordingId() + "/";
	...
	private Recording sealMetadataFiles(Recording recording) {
		...
		String folderPath = this.mediaServerProperties.recordingProperty.getRecordingPath() + recording.getId() + "/";
