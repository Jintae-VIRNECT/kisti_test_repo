...
import com.virnect.mediaserver.kurento.endpoint.EndpointType;
...
public abstract class SessionManager {
	...
	public void sendMessage(String message, String sessionId) {
		try {
			...
			throw new RemoteServiceException(Code.SIGNAL_FORMAT_INVALID_ERROR_CODE,
	 ...
	 * @throws RemoteServiceException in case the session doesn't exist
	 */
	public Set<Participant> getParticipants(String sessionId) throws RemoteServiceException {
		Session session = sessions.get(sessionId);
		if (session == null) {
			throw new RemoteServiceException(Code.ROOM_NOT_FOUND_ERROR_CODE, "Session '" + sessionId + "' not found");
		}
	 ...
	 * @throws RemoteServiceException in case the session doesn't exist or the
	 *                           participant doesn't belong to it
	 */
	public Participant getParticipant(String sessionId, String participantPrivateId) throws RemoteServiceException {
		Session session = sessions.get(sessionId);
		if (session == null) {
			throw new RemoteServiceException(Code.ROOM_NOT_FOUND_ERROR_CODE, "Session '" + sessionId + "' not found");
		}
	 ...
	 * @param participantPrivateId private identifier of the participant
	 * @return {@link Participant}
	 * @throws RemoteServiceException in case the participant doesn't exist
	 */
	public Participant getParticipant(String participantPrivateId) throws RemoteServiceException {
	...
	public Session storeSessionNotActive(String sessionId, SessionProperties sessionProperties) {
		Session sessionNotActive = new Session(sessionId, sessionProperties, mediaServerProperties, recordingManager);
	...
	public String newToken(Session session, RemoteServiceRole role, String serverMetadata,
	...
	public Token newTokenForInsecureUser(Session session, String token, String serverMetadata) throws Exception {
		Token tokenObj = new Token(token, RemoteServiceRole.PUBLISHER, serverMetadata != null ? serverMetadata : "",
				this.mediaServerProperties.coturnProperty.isTurnadminAvailable() ? this.coturnCredentialsService.createUser() : null, null);
		session.storeToken(tokenObj);
	...
	public boolean isPublisherInSession(String sessionId, Participant participant) {
		if (!this.isInsecureParticipant(participant.getParticipantPrivateId())) {
			if (this.sessionidParticipantpublicidParticipant.get(sessionId) != null) {
				return (RemoteServiceRole.PUBLISHER.equals(participant.getToken().getRole())
						|| RemoteServiceRole.MODERATOR.equals(participant.getToken().getRole()));
			} else {
				return false;
			}
		} else {
	...
	public boolean isModeratorInSession(String sessionId, Participant participant) {
		if (!this.isInsecureParticipant(participant.getParticipantPrivateId())) {
			if (this.sessionidParticipantpublicidParticipant.get(sessionId) != null) {
				return RemoteServiceRole.MODERATOR.equals(participant.getToken().getRole());
			} else {
				return false;
			}
		} else {
	...
	public Participant newParticipant(String sessionId, String participantPrivatetId, Token token,
		...
		if (this.sessionidParticipantpublicidParticipant.get(sessionId) != null) {
			...
			throw new RemoteServiceException(Code.ROOM_NOT_FOUND_ERROR_CODE, sessionId);
		}
	}
	...
	private void startSessionGarbageCollector() {
		if (mediaServerProperties.serverProperty.getSessionsGarbageInterval() == 0) {
		...
		new UpdatableTimerTask(() -> {
			...
			for (Iterator<Entry<String, Session>> iter = sessionsNotActive.entrySet().iterator(); iter.hasNext();) {
				...
				if (sessionExistsSince > (mediaServerProperties.serverProperty.getSessionsGarbageThreshold() * 1000)) {
					try {
		...
		}, () -> (long) (mediaServerProperties.serverProperty.getSessionsGarbageInterval() * 1000)).updateTimer();
		...
		log.info(
				...
				mediaServerProperties.serverProperty.getSessionsGarbageInterval(), mediaServerProperties.serverProperty.getSessionsGarbageThreshold());
	}
	 ...
	 *
	 * @param sessionId identifier of the session
	 * @throws RemoteServiceException in case the session doesn't exist or has been
	 *                           already closed
	 */
	public void closeSession(String sessionId, EndReason reason) {
		...
		if (session.isClosed()) {
			...
			throw new RemoteServiceException(Code.ROOM_CLOSED_ERROR_CODE, "Session '" + sessionId + "' already closed");
		}
