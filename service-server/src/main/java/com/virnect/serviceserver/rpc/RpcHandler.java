...
package com.virnect.serviceserver.rpc;
...
public class RpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	private void joinRoom(RpcConnection rpcConnection, Request<JsonObject> request) {
		...
		final com.virnect.serviceserver.core.Session session = sessionManager.getSessionWithNotActive(sessionId);
		if (session == null) {
	...
	public void afterConnectionClosed(Session rpcSession, String status) throws Exception {
		...
		if (!message.isEmpty()) {
			...
			if (rpc != null && rpc.getSessionId() != null) {
				com.virnect.serviceserver.core.Session session = this.sessionManager.getSession(rpc.getSessionId());
