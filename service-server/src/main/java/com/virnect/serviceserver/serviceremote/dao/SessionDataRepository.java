...
import com.virnect.data.dto.response.session.SessionResponse;
import com.virnect.data.dto.response.session.SessionTokenResponse;
...
public class SessionDataRepository {
	...
	public void setAccessStatus(Participant participant, String sessionId, AccessType accessType) {
		...
		try {
			...
			if (!ObjectUtils.isEmpty(member)) {
				...
				accessStatusService.saveAccessStatus(workspaceId, uuid, accessType);
			} else {
	...
	public Boolean leaveSession(Participant participant, String sessionId, EndReason reason) {
		...
		if (ObjectUtils.isEmpty(room)) {
			...
			throw new RemoteServiceException(ErrorCode.ERR_ROOM_NOT_FOUND);
		}
	...
	public ErrorCode joinSession(Participant participant, String sessionId) {
		...
		try {
			...
			throw new com.virnect.client.RemoteServiceException(
				com.virnect.client.RemoteServiceException.Code.USER_METADATA_FORMAT_INVALID_ERROR_CODE,
	...
	public Boolean disconnectSession(Participant participant, String sessionId, EndReason reason) {
		...
		try {
			...
			if (clientMetaData == null) {
				throw new RemoteServiceException(ErrorCode.ERR_SESSION_CLIENT_METADATA_NULL);
			}
			...
			throw new RemoteServiceException(ErrorCode.ERR_SESSION_CLIENT_METADATA_EXCEPTION);
		}
		return true;
	}
	 ...
	 */
	public void prepareJoinRoom(String workspaceId, String sessionId, String userId) {
		...
		if (!room.getMembers().isEmpty()) {
			long memberCount = room.getMembers().stream().filter(
				member -> !(member.getMemberStatus() == MemberStatus.UNLOAD
		...
		switch (room.getSessionProperty().getSessionType()) {
			...
			case PUBLIC: {
				if (!ObjectUtils.isEmpty(member)) {
					...
					if (memberStatus == MemberStatus.UNLOAD) {
						...
						throw new RemoteServiceException(ErrorCode.ERR_ROOM_MEMBER_EVICTED_STATUS);
					} else if (memberStatus == MemberStatus.LOAD) {
						throw new RemoteServiceException(ErrorCode.ERR_ROOM_MEMBER_ALREADY_JOINED);
					} else {
			...
			break;
			default: {
				throw new RemoteServiceException(ErrorCode.ERR_UNSUPPORTED_DATA_TYPE_EXCEPTION);
			}
		}
	}
	 ...
	 * Prepare to join the room the user is....
	 */
	public void prepareJoinRoomOnlyGuest(String workspaceId, String sessionId, String userId) {

		Room room = roomRepository.findOpenRoomByGuest(workspaceId, sessionId)
			.orElseThrow(() -> new RemoteServiceException(ErrorCode.ERR_ROOM_NOT_FOUND));

		if (!room.getMembers().isEmpty()) {
			long memberCount = room.getMembers().stream().filter(
				member -> !(member.getMemberStatus() == MemberStatus.UNLOAD
					|| member.getMemberStatus() == MemberStatus.EVICTED)).count();
			if (memberCount >= ROOM_MEMBER_LIMIT) {
				throw new RemoteServiceException(ErrorCode.ERR_ROOM_MEMBER_MAX_COUNT);
			}
		}
	...
	public KickRoomResponse kickFromRoom(
		String workspaceId,
		String sessionId,
	...
	public InviteRoomResponse inviteMember(
