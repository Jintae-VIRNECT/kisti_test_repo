...
@Profile({"local", "onpremise", "develop", "freezing"})
@Component
...
public class LocalFileManagementService implements IFileManagementService {
	...
	private List<String> setFileAllowExtensionList(JsonObject policyObject) {
		...
		try {
			List<String> audioList = objectMapper.readValue(
				resourceObject.getAsJsonArray("audio").toString(), new TypeReference<List<String>>() {
				});
			fileAllowExtensionList.addAll(audioList);

			List<String> compressedList = objectMapper.readValue(
				resourceObject.getAsJsonArray("compressed").toString(), new TypeReference<List<String>>() {
				});
			fileAllowExtensionList.addAll(compressedList);

			List<String> docEtcList = objectMapper.readValue(
				resourceObject.getAsJsonArray("document_etc").toString(), new TypeReference<List<String>>() {
				});
			fileAllowExtensionList.addAll(docEtcList);

			List<String> docMSList = objectMapper.readValue(
				resourceObject.getAsJsonArray("document_hwp").toString(), new TypeReference<List<String>>() {
				});
			fileAllowExtensionList.addAll(docMSList);

			List<String> docHWPList = objectMapper.readValue(
				resourceObject.getAsJsonArray("document_ms").toString(), new TypeReference<List<String>>() {
				});
			...
			List<String> executeList = objectMapper.readValue(
				resourceObject.getAsJsonArray("executable").toString(), new TypeReference<List<String>>() {
				});
			fileAllowExtensionList.addAll(executeList);

			List<String> fileModelList = objectMapper.readValue(
			...
			List<String> videoList = objectMapper.readValue(
				resourceObject.getAsJsonArray("video").toString(), new TypeReference<List<String>>() {
				});
	...
	@Override
	public UploadResult upload(MultipartFile file, String dirPath, FileType fileType) throws
		IOException,
	...
	@Override
	public UploadResult objectFileUpload(
		InputStream inputStream, String dirPath, String fileOriginalName, String objectName, String objectFilePath
	) throws
		...
		//String objectName = String.format("%s_%s", LocalDate.now(), RandomStringUtils.randomAlphabetic(20));
		try {
			objectPath.append(dirPath)
				.append(fileBucketName)
				.append("/")
				.append(FilenameUtils.getBaseName(objectFilePath))
				.append("/")
				.append(objectName);
	...
	@Override
	public UploadResult uploadProfile(MultipartFile file, String dirPath) throws
		IOException,
		NoSuchAlgorithmException,
		InvalidKeyException {
		// check file is null
	...
	@Override
	public boolean removeObject(String objectPathToName) throws
		IOException,
		NoSuchAlgorithmException,
		InvalidKeyException {
		try {
	...
	@Override
	public void deleteProfile(String objectPathToName) throws
		IOException,
		NoSuchAlgorithmException,
		InvalidKeyException {
	...
	public void removeBucket(String bucketName, String dirPath, List<String> objects, FileType fileType) throws
		IOException,
		NoSuchAlgorithmException,
		InvalidKeyException {
			...
			switch (fileType) {
				...
				case OBJECT:
					objectPath.append(dirPath)
						.append(fileBucketName)
						.append("/")
						.append(FilenameUtils.getBaseName(objectName))
						.append("/")
						.append(objectName);
					url = minioClient.getObjectUrl(bucketName, objectPath.toString());
					LogMessage.formedInfo(
