			...
			try {
				...
				if (e.code === 4002)
					return this.alertMessage(
						'계정정보 불일치', // 계정정보 불일치
						'입력하신 정보와 일치하는 VIRNECT 회원이 없습니다.', // 입력하신 정보와 일치하는 VIRNECT 회원이 없습니다.
						'error',
					)
				else
					return this.alertMessage(
						'계정정보 전송 오류', // 계정정보 전송 오류
						'계정정보 전송에 실패하였습니다. 잠시 후 다시 이용해 주세요.', // 계정정보 전송에 실패하였습니다. 잠시 후 다시 이용해 주세요.
						'error',
					)
			}
		},
			...
			try {
				...
				if (res.code === 200)
					return this.confirmWindow(
						...
						'기존 로그인된 기기에서 로그아웃 됩니다. 변경된 새 비밀번호로 다시 로그인해 주세요.', // 기존 로그인된 기기에서 로그아웃 됩니다. 변경된 새 비밀번호로 다시 로그인해 주세요.
						'확인', // 확인
					)
				else throw res
			} catch (e) {
				if (e.code === 4009)
					return this.alertMessage(
						'비밀번호 재설정 실패', // 비밀번호 재설정 실패
						'이전과 동일한 비밀번호는 새 비밀번호로 설정할 수 없습니다.', // 이전과 동일한 비밀번호는 새 비밀번호로 설정할 수 없습니다.
						'error',
					)
				else
					return this.alertMessage(
		...
		async emailPassCode() {
			try {
				...
				if (res.code === 200) {
					this.alertMessage(
						'보안코드 전송', // 보안코드 전송
						'가입하신 이메일로 보안 코드가 전송되었습니다. 이메일의 보안 코드를 확인하여 입력해 주세요.', // 가입하신 이메일로 보안 코드가 전송되었습니다. 이메일의 보안 코드를 확인하여 입력해 주세요.
						'success',
					)
				...
				if (e.code === 4002)
					...
					return this.alertMessage(
						'보안코드 전송 오류', // 보안코드 전송 오류
						'보안코드 이메일 전송에 실패하였습니다. 잠시 후 다시 이용해 주세요.', // 보안코드 이메일 전송에 실패하였습니다. 잠시 후 다시 이용해 주세요.
						'error',
					)
			}
		},
		async authCodeCheck() {
			try {
				...
				if (res.code === 200) {
					...
					this.alertMessage(
						'보안코드 일치', // 보안코드 일치
						'보안 코드 인증이 완료되었습니다.', // 보안 코드 인증이 완료되었습니다.
						'success',
					)
				...
				if (e.code === 4007)
					return this.alertMessage(
						'보안코드 불일치 오류', // 보안코드 불일치 오류
						'입력하신 보안 코드가 일치하지 않습니다. 다시 한 번 확인해 주세요.', // 입력하신 보안 코드가 일치하지 않습니다. 다시 한 번 확인해 주세요.
						'error',
					)
				else
					return this.alertMessage(
